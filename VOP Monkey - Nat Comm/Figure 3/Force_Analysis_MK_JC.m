...
Hz = {'IC alone', 'VOP 50 Hz', 'VOP 100 Hz'};
forces = {};
...
for t=1:length(TrialsToLoad)
    ...
    for aa = 1:skip:length(StimTimes{1,t})
        ...
        if upperbound < length(forces{1,t}(1,:)) && lowerbound >= 0
            ...
            area = trapz(magForce{t}(count,:));
            auc_f{t}(count,:) = area;

            p2p_t{t}(count,:) = peak2peak(magTorque{t}(count,:));
            area = trapz(magTorque{t}(count,:));
            auc_t{t}(count,:) = area;
...
[ci95, rejectNull] = bootstrapCompMeans(auc_f{1}, auc_f{2}, 10000,.001)
