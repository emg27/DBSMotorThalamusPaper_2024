...
% TrialsToLoad=[9 11 12 13];
% Hz = {'IC PRE', '50 Hz', '80 Hz','100Hz'};

TrialsToLoad=[9 181 189 188 187];
...
% % Hz = {'IC POST', '50 Hz', '80 Hz','100Hz'};
% Hz = {'IC POST', '50 Hz', '80 Hz', '100 Hz'};
...
%         p2pCell{t} = rmoutliers(singleP2P(aa,:)');
...
%         [CI,sig]=bootstrapCompMeans((p2p{1,trial1}(aa,:)),(p2p{1,trial2}(aa,:)),10000,alpha/size(trialMat,1));
...
% %% Percent Variation
emgName = {'EDC','ECR','FCR','FDC','ABP','FDM','BIC','SupLip','Cheek','Jaw'};
meanp2p = [];
meanauc = [];
...
%% PANEL B: (Right) COMBINED PERCENT INCREASE
...
%     legend('IC alone', 'VOP 50 Hz', 'VOP 100 Hz')
    linkaxes(axes,'xy')
    hold off
    saveas(gcf,['MK-HS_postLesion_EMGtrace_' emgName{chn}])
end
