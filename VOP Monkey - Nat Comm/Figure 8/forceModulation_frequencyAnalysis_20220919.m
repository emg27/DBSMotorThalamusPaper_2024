...
trials = [15 23];
pathname = 'P:\projects\human\VOP STIM\DATA\Chronic_DBS\BP\20220919\';
...
% trialStart = [697413 980458]; %20220923
trialStart = [883587 727172]; %20220919

for aa = 1:length(data_ns5)

    wholeTrace{aa} = filter_data(data_ns5{aa}.Data(1,trialStart(aa):trialStart(aa)+T*Fs));
%     wholeTrace{aa} = (data_ns5{aa}.Data(1,trialStart(aa):trialStart(aa)+T*Fs));
    figure;
    plot(wholeTrace{aa})
    figure(3)
    hold on
    Y = fft(wholeTrace{aa});
    L = length(wholeTrace{aa});
...
for n = 1:length(fftMat)
    ...
    seg{n} = reshape(fftMat{n}(:,window),1,[])';
