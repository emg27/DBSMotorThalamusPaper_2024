...
pathName = 'D:\Figures\VOP_NatCom_2024_Data\Sup Fig 6\';
trialsToLoad = [103 111 113];

% bodypartlst = {'thumb','index','wrist_M','wrist_L','elbow'};
 bodypartlst = {'thumb','index','wrist_M','elbow'};

%% Load Data

for t = 1:length(trialsToLoad)
     dlc_filename = sprintf('PAD%03d',trialsToLoad(t));
...
%        end
        ...
        [pks,locs] = findpeaks(singleTrace,'MinPeakDistance',1,'MinPeakProminence',1.3);
 ...
 for t = 1:size(traces,2)
   for aa = 1:size(cleanedTraces(t,:),2)
       for ii  = 1:size(cleanedTraces{t,aa})
           p2p = peak2peak(cleanedTraces{t,aa}(ii,:));

           amps{t,aa}(ii) = p2p;
       end
       amps{t,aa} = rmoutliers(amps{t,aa},'median');
%        amps{t,aa} = amps{t,aa};
   end
 end
...
for shift = 1:2
    ...
    for aa = shift:2:length(bodypartlst)*2
        ...
        if shift ==1
            ...
            title(['Subzero POST ' bodypartlst{count} axis{shift}])
...
% part_y=15;
    ...
    nostim_x = data{1,nostim}(:,part_x)-mean(data{1,nostim}(:,part_x))+10;
 
    hz50_x = data{1,hz50}(:,part_x)-mean(data{1,hz50}(:,part_x))+20;
    hz100_x = data{1,hz100}(:,part_x)-mean(data{1,hz100}(:,part_x))+25;

    nostim_y = data{1,nostim}(:,part_y)-mean(data{1,nostim}(:,part_y))+10;
   
    hz50_y = data{1,hz50}(:,part_y)-mean(data{1,hz50}(:,part_y))+20;
    hz100_y = data{1,hz100}(:,part_y)-mean(data{1,hz100}(:,part_y))+25;

    tiledlayout(1,2)
    time = (1:length(nostim_x))/24;
  
    time50 = (1:length(hz50_x))/24;
    time100 = (1:length(hz100_x))/24;

    parameters = {time,time50,time100,nostim_x,hz50_x,hz100_x,nostim_y,hz50_y,hz100_y,bodypart};
...
%     figure;
    ...
    plot(time10,hz10_x,'green')
    plot(time50,hz50_x,'cyan')
    plot(time100,hz100_x, 'black')
    legend('No Stim','50 Hz','100 Hz')
    hold off
